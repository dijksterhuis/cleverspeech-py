...
class MinimumEnergyPatch(Path):
    ...
    def minimum_energy_patched(self, t_mod, sig, af, sample_rate=16000):
        ...
        window_step = int(0.02 * sample_rate)

        frames = psf.sigproc.framesig(sig, window_size, window_step)
        n_positions = len(frames) - t_len

        def energy(x):
            return np.sum(np.square(np.abs(x)))

        frame_energies = l_map(
            lambda i: energy(frames[i: i + t_len]),
            range(n_positions)
        )

        argmin = np.argmin(frame_energies)

        path[argmin:argmin + t_len] = t_mod
...
class MinimumEnergyDistributed(Path):
    ...
    def minimum_energy_distributed(self, t_mod, sig, af, sample_rate=16000):
        ...
        frame_nrg = [np.sum(np.square(np.abs(frame))) for frame in frames]
        lowest_energies = list(sorted(frame_nrg))[:t_len]
...
ALL_PATHS = {
    ...
    "patch-mid": MidPatch,
    "patch-end": EndPatch,
    "patch-minnergy": MinimumEnergyPatch,
    "dist-minnergy": MinimumEnergyDistributed,
}
