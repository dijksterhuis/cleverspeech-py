...
def generate_all_possible_etls(settings):

    import itertools

    audios = MCV1SilenceAudioBatchETL(
        settings["audio_indir"],
        settings["max_examples"],
        filter_term=".wav",
        max_file_size=settings["max_audio_file_bytes"],
    )
    transcriptions = MCV1TranscriptionsFromCSVFile(
        settings["targets_path"],
        settings["max_targets"],
    )
    batch_gen = MCV1IterableBatches(
    ...
    iterable = itertools.product(output_dtypes, non_silence)

    for output_type, audio_etl in iterable:
        audios = audio_etl(
            settings["audio_indir"],
            settings["max_examples"],
            filter_term=".wav",
