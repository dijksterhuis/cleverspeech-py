...
def carlini_and_wagner_stt_simple_graph(sess, batch, settings):
    ...
    attack.add_loss(
        graph.Losses.BEAM_SEARCH_ADV_LOSSES[settings["loss"]],
        weight_settings=(1.0, 1 / settings["importance"]),
        updateable=True,
    )
    # attack.add_loss(
    #     graph.Losses.L2TanhSquaredLoss,
    #     weight_settings=(1.0, settings["importance"]),  # (1e-2, 1.5)
    #     updateable=True,
    # )
    attack.add_loss(
...
def attack_run(master_settings):
    ...
    if attack_type == "cw-simple":

        master_settings["weights_loss_1"] = (
            1.0, master_settings["importance"])

        master_settings["weights_loss_2"] = (
            1.0e-3, 1
        )
...
def main():
    ...
    extra_args = {
        ...
        "importance": [float, 2.0, False, None],
